...
A+ Setup Filters (Task B tuned for crypto):
- ADX < max threshold (ranging market — mean reversion fails in strong trends)
- ATR ratio >= min× average (some volatility, not dead)
- RSI oversold / overbought per dataclass defaults
"""
...
class MeanReversionConfig:
    ...
    atr_stop_mult: float = 1.5  # Stop distance = ATR * this multiplier (tighter stops for mean reversion)
    atr_period: int = 14  # ATR calculation period
    stop_buffer_ATR: float = 0.15  # Buffer outside Bollinger Bands (in ATR units)
...
def get_config_schema() -> Dict[str, Any]:
    ...
    return {
        ...
        "description": (
            ...
            "CRITICAL: Only signals in ranging markets (ADX below configured max)."
        ),
    }
