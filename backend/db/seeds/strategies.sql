-- Strategy seed data for Omni-Bot Trading Platform
-- Authoritative source: contracts/types.md (StrategyConfig)
-- Updated: Replaced old strategies with new production-grade analyzed strategies
-- Idempotent: Uses ON CONFLICT DO UPDATE to allow re-running

-- Strategy 1: VWAP Mean Reversion (15m entry, 1h filter)
...
-- Strategy 2: Volatility Contraction → Expansion (15m entry, 1h filter)
...
-- Strategy 3: HTF Trend Pullback Continuation (1h entry, 4h trend)
...
-- Strategy 4: VWAP Mean Reversion 1h (same class/logic as vwap meanrev; higher bar frequency)
-- Mirrors backend/db/migrations/005_add_vwap_meanrev_1h.sql; DO NOTHING avoids overwriting tuned rows.
INSERT INTO strategies (name, config, status)
VALUES (
    'vwap_meanrev_1h',
    jsonb_build_object(
...
-- Deactivate old strategies (keep for reference but set to inactive)
